 ...
 */
export function extractHeartRate(signals: number[][], timestamps: number[]): { heartRate: number; confidence: number } {
  ...
  try {
    ...
    const { dominantFrequency, signalStrength } = performFrequencyAnalysis(filteredSignal, samplingRate)
    ...
    console.error("Error processing rPPG signal:", error)
    return defaultResult
  }
}
 ...
 */
function bandpassFilter(signal: number[], samplingRate: number, lowCutoff: number, highCutoff: number): number[] {
