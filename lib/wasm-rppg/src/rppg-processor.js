...
class RPPGProcessor {
      ...
      loadOpenCV(() => {
        try {
          ...
          if (isBrowser) {
            ...
            // Node.js 환경에서는 opencv-haarcascade 패키지를 사용
  ...
  calculateHeartRate() {
    ...
    try {
      ...
      // S와 X의 행렬곱 계산
      const P = [
        this._dotProduct(S[0], X),
        this._dotProduct(S[1], X)
      ];
      ...
      // 필터링
      const filteredSignal = this._bandpassFilter(posSignal, this.samplingRate, 0.7, 4.0);
      
      // FFT를 이용한 주파수 분석
      const { dominantFrequency, signalStrength } = this._performFrequencyAnalysis(filteredSignal, this.samplingRate);
      ...
      // R-R 간격 계산을 위한 피크 감지
      const peaks = this._findPeaks(filteredSignal, this.samplingRate, heartRate);
  ...
  _ifft(fftResult) {
    ...
    for (let t = 0; t < n; t++) {
      ...
      for (let k = 0; k < n; k++) {
        const angle = (2 * Math.PI * t * k) / n;
        val += fftResult[k].real * Math.cos(angle) - fftResult[k].imag * Math.sin(angle);
      }
  ...
  _performFrequencyAnalysis(signal, samplingRate) {
    ...
    // 진폭 스펙트럼 계산
    const amplitudeSpectrum = fft.map(val => Math.sqrt(val.real * val.real + val.imag * val.imag));
    
    // 관심 주파수 범위 (0.7Hz ~ 4Hz, 약 42~240 BPM)
    const n = signal.length;
    const minFreqIdx = Math.floor((0.7 * n) / samplingRate);
    const maxFreqIdx = Math.min(Math.ceil((4.0 * n) / samplingRate), Math.floor(n / 2));
    ...
    const totalPower = amplitudeSpectrum.slice(minFreqIdx, maxFreqIdx + 1)
  ...
  _findPeaks(signal, samplingRate, heartRate) {
    ...
    // 극대값 찾기
    for (let i = 1; i < n - 1; i++) {
      if (signal[i] > signal[i - 1] && signal[i] > signal[i + 1] && // 로컬 최대값
          signal[i] > threshold && // 임계값 초과
          i - lastPeakIdx >= minDistance) { // 최소 거리 조건 충족
        peaks.push(i);
        lastPeakIdx = i;
      }
    }
  ...
  _calculateHRV(peakIndices, timestamps) {
    ...
    for (let i = 0; i < peakIndices.length - 1; i++) {
      const rr = (timestamps[peakIndices[i + 1]] - timestamps[peakIndices[i]]) * 1000;
      rrIntervals.push(rr);
    }
    
    // 이상치 제거 (45%-155% 범위 밖의 RR 간격 제거)
    const meanRR = this._mean(rrIntervals);
    const validRR = rrIntervals.filter(rr => rr >= 0.45 * meanRR && rr <= 1.55 * meanRR);
  ...
  _calculateFrequencyDomainHRV(rrIntervals) {
    try {
      ...
      const spectrumResult = this._welch(resampledRR, window, noverlap, nfft, fs);
      const { frequencies, psd } = spectrumResult;
      
      // LF와 HF 대역 찾기
      const lfIndices = frequencies.map((f, i) => f >= 0.04 && f <= 0.15 ? i : -1).filter(i => i !== -1);
      const hfIndices = frequencies.map((f, i) => f > 0.15 && f <= 0.4 ? i : -1).filter(i => i !== -1);
  ...
  _hammingWindow(length) {
    return Array.from({ length }, (_, n) => 0.54 - 0.46 * Math.cos((2 * Math.PI * n) / (length - 1)));
  }
